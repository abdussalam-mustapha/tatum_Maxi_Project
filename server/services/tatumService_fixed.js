...
export class TatumService {
  constructor() {
    ...
    // Use working Windows MCP client for better compatibility
    if (process.platform === 'win32') {
      this.mcpClient = new WorkingWindowsMCP()
      console.log('🪟 Using working Windows MCP client')
    } else {
  ...
  async getPortfolioDataDirect(address) {
    ...
    for (const chain of chains) {
      try {
        ...
        if (chainData.balance > 0 || (chainData.tokens && chainData.tokens.length > 0)) {
